...
public class ShoppingListAggregatorEngineTests {
    ...
    public void testAggregateIngredients() throws Exception {
        ...
        i1.setGrocery(g1);
        i1.setId(1L);
        i1.setQuantity(new Quantity(Unit.KG, 1));

        Ingredient i2 = new Ingredient();
        i2.setGrocery(g2);
        i2.setId(2L);
        i2.setQuantity(new Quantity(Unit.KG, 2));

        Ingredient i3 = new Ingredient();
        i3.setGrocery(g1);
        i3.setId(3L);
        i3.setQuantity(new Quantity(Unit.KG, 0.5));
        ...
        assertEquals(Unit.KG, aggregated.get(0).ingredient.getQuantity().getUnit());

        assertEquals(2, aggregated.get(1).ingredient.getQuantity().getQuantity(), 0);
        assertEquals(Unit.KG, aggregated.get(1).ingredient.getQuantity().getUnit());
